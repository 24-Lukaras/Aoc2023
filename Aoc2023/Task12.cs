...
namespace Aoc2023
{
    ...
    public class SpringLinePlatinum
    {
        Regex sliceRegex = new Regex(@"^(\?|#)+$");
        Regex firstMatchRegex = new Regex(@"[#]+");
        ...
        public int TryFit(int start, int index, List<SpringLineOverlap> results, params int[] lengths)
        {
            if (index >= lengths.Length)
            {
                var matches = firstMatchRegex.Matches(Value);
                ...
                if (correct)
                {
                    var overlap = new SpringLineOverlap()
                    {
                        Value = Value,
                        OverlapStart = matches[matches.Count / 2 - 1].Index,
                        OverlapEnd = matches[matches.Count / 2].Index + matches[matches.Count / 2].Length,
                    };
                    if (matches[matches.Count / 2].Index < Value.Length / 2)
                    {
                        ;
                    }
                    results.Add(overlap);
            ...
            for (int i = start; i + lengths[index] <= Value.Length; i++)
            {
                ...
                if ((i + lengths[index] + 1 >= Value.Length || "?.".IndexOf(Value[i + lengths[index]]) != -1) && (i == 0 || Value[i - 1] != '#'))
                {
                    if (sliceRegex.IsMatch(slice))
                    {
                        int l = i + lengths[index] + 1 > Value.Length ? i + lengths[index] : i + lengths[index] + 1;
                        string newString = $"{Value.Remove(i, lengths[index]).Insert(i, string.Join(string.Empty, Enumerable.Repeat('#', lengths[index]))).Substring(0, l).Replace("?", ".")}{Value.Substring(l)}";
                        result += new SpringLinePlatinum()
                        {
                            Value = newString
                        }.TryFit(i + lengths[index], index + 1, results, lengths);
                    }
                }
    ...
    public class SpringLineOverlap
    {
        public string Value { get; set; }
